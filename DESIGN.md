...
- 버튼 안의 아이콘과 텍스트 간격은 `0.5rem` 기준으로 유지합니다.
- 버튼 폭은 내용 중심을 기본으로 하고, 전체 폭 버튼은 필요한 경우에만 사용합니다.
...
- 입력 필드 내부 패딩과 높이는 Bootstrap 기본값을 우선 사용합니다.
- 도움말 텍스트와 오류 메시지는 본문보다 한 단계 작은 크기를 사용합니다.
...
- 테이블 헤더는 기본적으로 좌측 정렬을 유지합니다.
- 숫자형 데이터는 우측 정렬을 우선합니다.
- 정렬 가능한 컬럼은 시각적 정렬 상태를 명확히 표시합니다.
