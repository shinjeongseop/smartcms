...
공통화 대상은 `head/foot`, 공통 헬퍼, 공통 UI 규칙, 공통 CSS이며, `view.php`를 하나로 합치는 공통 뷰 방식은 채택하지 않는다.
...
| Template structure | 공통 view 하나 / 스킨별 view 독립 | 스킨별 `list/view/form` 독립 | 갤러리, 유튜브, 기본게시판의 화면 차이를 유지하면서 수정 용이성 확보 |
