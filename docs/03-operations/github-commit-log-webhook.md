...
- 토큰은 코드에서 만들지 않고, GitHub Secrets에 저장한 뒤 배포 시 서버 런타임 파일로 전달한다.
- 같은 값을 GitHub Actions와 서버가 함께 사용해야 한다.
- 토큰은 길고 예측 불가능한 임의 문자열로 만든다.
...
1. 새 토큰을 생성한다.
2. GitHub 저장소의 `SMARTCMS_WEBHOOK_TOKEN` Secret을 새 값으로 바꾼다.
3. 서버의 `webhook.local.php`에 반영되도록 배포를 다시 실행한다.
4. 이전 토큰은 더 이상 사용하지 않도록 폐기한다.
...
- 토큰은 저장소에 커밋하지 말고 Secrets와 런타임 파일로만 관리한다.
