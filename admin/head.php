...
<?php if (!$is_login_page): $admin = smartcms_current_user(); ?>
  <div class="d-flex min-vh-100">
    <!-- [SIDEBAR] 어드민 사이드바 -->
    <aside class="offcanvas-md offcanvas-start sc-admin-sidebar d-flex flex-column bg-white border-end p-3 flex-shrink-0 shadow-sm" tabindex="-1" id="adminSidebarOffcanvas" aria-labelledby="adminSidebarTitle">
      <header class="d-flex align-items-center justify-content-between mb-4 px-2">
        ...
        <button type="button" class="btn-close d-md-none" data-bs-dismiss="offcanvas" data-bs-target="#adminSidebarOffcanvas" aria-label="관리자 메뉴 닫기"></button>
      </header>

      <nav class="nav nav-pills flex-column gap-1 flex-grow-1 overflow-auto" aria-label="관리자 메뉴">
        ...
        <?php foreach (smartcms_admin_nav_items() as $key => $item): ?>
          <a class="nav-link d-flex align-items-center gap-2 py-2 px-3 rounded-2 <?= $key === $active_menu ? 'active fw-bold' : 'text-secondary' ?>" href="<?= smartcms_h($item['href']) ?>" <?= $key === $active_menu ? 'aria-current="page"' : '' ?>>
    ...
    <div class="flex-grow-1 d-flex flex-column bg-light sc-admin-workspace">
      <header class="bg-white border-bottom px-3 px-md-4 py-3 sticky-top z-1 shadow-sm sc-admin-topbar">
        <div class="d-flex align-items-center justify-content-between gap-3">
          <div class="d-flex align-items-center gap-2 gap-sm-3 min-w-0">
            <button class="btn btn-light border d-md-none shadow-none p-2 lh-1 flex-shrink-0" type="button" data-bs-toggle="offcanvas" data-bs-target="#adminSidebarOffcanvas" aria-label="관리자 메뉴 열기">
              <i class="bi bi-list fs-2"></i>
            </button>
            <nav aria-label="breadcrumb" class="d-none d-sm-block min-w-0">
              ...
              <h1 class="h4 fw-bold mb-0 text-dark text-truncate"><?= smartcms_h($page_heading) ?></h1>
            </nav>
            <h1 class="h5 fw-bold mb-0 text-dark d-sm-none text-truncate"><?= smartcms_h($page_heading) ?></h1>
          </div>
          <div class="d-flex gap-2 flex-shrink-0">
            <a class="btn btn-light border btn-sm rounded-2 px-3 fw-bold shadow-none" href="/" aria-label="사이트 홈">
